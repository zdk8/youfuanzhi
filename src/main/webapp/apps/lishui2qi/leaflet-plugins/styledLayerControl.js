L.Control.StyledLayerControl = L.Control.Layers.extend({
		...
		_initLayout : function () {
			...
			for(var c = 0; c < (containers = $(container).find('ac-container')).length; c++ ){
			...
			window.onresize = function(){
				me._on_resize_window.call(me);
			}

				//this._on_resize_window.bind(this);
		...
		_addLayer : function (layer, name, group, overlay) {
			...
			if (group) {
				...
				var groupId = _myindexOf(this._groupList,group);
		...
		_addItem : function (obj) {
			...
			if (!groupContainer) {
				...
				inputLabel   = '<label for="ac' + obj.group.id + '" class="myac" >' + obj.group.name + '</label>';
				...
				$(groupContainer).children().last().get(0).appendChild(label);
			}	
